                ...
                return (
                    <Link to={item.link} className={`link-SideBar ${item.class}`} key={index}>
                        <li className={`link-SideBar ${item.class} ${isActive ? 'active-link' : ''} tran`}>
                            <FontAwesomeIcon icon={item.icon} className="col-2 pl-5" style={{ fontSize: '1.5em' }} />
                            <span className="col-8 d-none d-xl-block">{item.title}</span>
                            {item.hint && <span className="d-flex justify-content-center align-items-center text-light rounded-2 bg-danger hintSideBar" style={{fontSize:'12px'}}>{numberOfMessage}</span>}
                        </li>
                    </Link>
                );
            })}
